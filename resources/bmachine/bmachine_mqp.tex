...
% include democracy player icon here
...
% let's get some screen shots for here too
...
The Internet Center Theme, the theme for http://www.openwebdesign.org}, is an example of a theme that was built using PHPTemplate. Below is a snippet of code that demonstrates how PHPTemplate looks. 

<div class="logo">

        <?PHP if (\$site\_name) : ?>

            <?PHP if (\$site\_name) \{ ?><h1 class='site-name'><a href=\"<?PHP print \$base\_path ?>\"\ title=\"<?PHP print t('Home') ?>\"><?PHP print \$site\_name ?></a></h1><?PHP \} ?>
...
Bluebeach, the theme used by http://drupal.org} was originally written as a XTemplate theme, but was recently ported to be a Smarty-based theme. 
...
They believe that "modularity better suits the open-source development model, because otherwise you can't easily have people working in parallel without risk of interference"\footnote{http://drupal.org/node/292}. % Jan/30/07: that's a proof by contradiction. :) -drew (feel free to kill this comment after you read it)
...
The Smarty engine is distribute as a tarball from http://smarty.php.net}. 
...
\subsection {Wordpress and Movable Type}
...
%Jan 31: shouldn't this be cut? -drew
...
\subsection{Customizable} %Jan/30/07: What's a better name for this requirement? -drew
...
\section{URL Rewriting (mod\_rewrite)}
...
It will only download your MP3 if it looks like http://www.foobar.com/index.mp3?show=4.mp3} versus the traditional http://www.foobar.com/index.mp3?show=4.mp3}. \footnote{http://www.apple.com/itunes/store/podcaststechspecs.html}
Not convenient, but necessary given the popularity of iTunes and the demand for compatible content.


\section{BitTorrent}
% 1/23/2007, CJ: Draft finished.
...
\begin{description}
\item[info\_hash] A 20-byte SHA1 hash of the value of the info key that was sent in the torrent file.
\item[peer\_id] A 20-byte string used to identify the client.
...
% 2/1/2007, CJ: Add something about the creative commons module.
